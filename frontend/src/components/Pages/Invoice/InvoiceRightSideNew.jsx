...
function InvoiceRightSideNew() {
    ...
    if (Object.keys(validationErrors).length === 0) {
      try {
        ...
        if (res.data.length > 0) {
          // Calculate how many of this item have already been added to the cart in this session
          const newlyAddedQty = (invoiceObject?.eqdetails || [])
            .filter((item) => item.eq_id == equipment.eq_id)
            .reduce(
              (sum, item) => sum + parseInt(item._addedInSessionQty || 0, 10),
              0,
            );

          const availableStock =
            equipment.eq_inhand_qty -
            equipment.eq_defected_status -
            newlyAddedQty;

          if (availableStock > 0) {
            setEqQuantity(availableStock);
            ...
            setEqName(equipment.eq_name + " (Out of Stock)");
            setEqQuantity(0);
  ...
  const handleSubmit = (e) => {
    ...
    if (Object.keys(validationErrors).length === 0) {
      ...
      const existingItemIndex = (invoiceObject?.eqdetails || []).findIndex(
        (item) => item.eq_id == eqFullDetail.eq_id,
      );

      if (existingItemIndex !== -1) {
        const updatedInvoiceObject = { ...invoiceObject };
        const updatedEqDetails = [...updatedInvoiceObject.eqdetails];
        const existingItem = { ...updatedEqDetails[existingItemIndex] };

        existingItem.inveq_borrowqty =
          parseInt(existingItem.inveq_borrowqty || 0, 10) + quantityToAdd;
        existingItem._addedInSessionQty =
          parseInt(existingItem._addedInSessionQty || 0, 10) + quantityToAdd;

        updatedEqDetails[existingItemIndex] = existingItem;
        updatedInvoiceObject.eqdetails = updatedEqDetails;

        updateEqObject({
          ...existingItem,
          inveq_borrowqty: existingItem.inveq_borrowqty,
        });
        setInvoiceObject(updatedInvoiceObject);
      } else {
        const newItem = { ...eqFullDetail };
        newItem.inveq_borrowqty = quantityToAdd;
        newItem.inveq_borrow_date = dateformatter();
        newItem._addedInSessionQty = quantityToAdd;

        updateEqObject({ ...newItem, inveq_borrowqty: quantityToAdd });
        updateValue("eqdetails", newItem);
      }
      handleReset();
    }
  };
